...
class Juz {
  Juz({required this.juz});

  List<JuzElement> juz;

  factory Juz.fromJson(Map<String, dynamic> json) => Juz(
    juz: List<JuzElement>.from(json["juz"].map((x) => JuzElement.fromJson(x))),
  );
...
class JuzElement {
  JuzElement({required this.number, required this.name, required this.surat});
